...
-- Entity Name: Encryption subkey calculator
-- Description: Calculates subkey for XTEA encryption
--              (sum + *k[sum & 3]) and (sum + *k[sum>>11 & 3]) depending on operation half
entity subkey_calc_enc is
    Port (clk:                  IN  STD_LOGIC;
          ...
          -- Control signal for encryption FSM
          -- Goes high for a clock cycle when xtea_enc has used the current subkey
          enc_subkey_enable:        IN STD_LOGIC;
          
          -- Output subkey calculation
          enc_key_word_out:     OUT unsigned(31 downto 0)
          

    );
end subkey_calc_enc;

architecture Behavioral of subkey_calc_enc is
    -- Declare constants for sum and delta
    constant delta:             unsigned(31 downto 0) := x"9E3779B9";
    constant sum0:              unsigned(31 downto 0) := x"00000000";
...
begin
    ...
    begin
        ...
        -- Positive-edge clocked
            ...
            elsif enc_subkey_enable = '1' then
    ...
    key_mux_select <= sum_reg(1 downto 0) when operation_half_reg = '0' else
                      sum_reg(12 downto 11);
    ...
    enc_key_word_out <= sum_reg + selected_key;
