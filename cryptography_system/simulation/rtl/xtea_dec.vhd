...
-- Entity Name: XTEA Decryption
-- Description: Contains decryption logic, calculates decrypted value with given subkey
--              (dx << 4 ^ dx >> 5) + dx) ^ subkey
entity xtea_dec is
    Port (clk:                  IN  STD_LOGIC;
          ...
          -- Decryption subkey in
          dec_key_word_in:      IN  unsigned(31 downto 0);
          
          -- Ciphertext in flag from testbench stimulus
          data_valid:           IN  STD_LOGIC;
          
          -- Ciphertext data in from testbench stimulus
          data_word_in:         IN  unsigned(31 downto 0);
          
          -- Subkey enable flag
          dec_subkey_enable:    OUT STD_LOGIC;
...
architecture Behavioral of xtea_dec is
    ...
    -- 128-bit ciphertext in 32-bit registers
    ...
    -- Register select flag for ciphertext in
    signal write_words_reg:             unsigned(1 downto 0);
    
    -- Register select flag for plaintext out
...
begin
            ...
            case state_reg is
               ...
               -- IDLE FSM State
                ...
                when ROUNDS =>
                    ...
                    -- First half of decryption round
                    ...
                    if round_half_reg = '0' then
                        ...
                        z0_reg <= z0_reg - (f0 xor dec_key_word_in);
                        z1_reg <= z1_reg - (f1 xor dec_key_word_in);
                    ...
                    -- Second half of decryption round
                    ...
                    -- y0  -= ((z0 << 4 ^ z0 >> 5) + z0) ^ (sum + *k[sum & 3]);
                        ...
                        y0_reg <= y0_reg - (f0 xor dec_key_word_in);
                        y1_reg <= y1_reg - (f1 xor dec_key_word_in);
    ...
    dec_subkey_enable   <= '1' when state_reg = ROUNDS else '0';
end Behavioral;
